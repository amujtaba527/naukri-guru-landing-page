            ...
            >
              <div className="flex items-center gap-1 mb-4">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} className="h-5 w-5 fill-naukri-amber text-naukri-amber transition-transform duration-300 group-hover:scale-110" style={{ animationDelay: `${i * 0.1}s` }} />
                ))}
              </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-2">
                {['AG', 'KL', 'DM', 'RS'].map((initials, i) => (
                  <div key={i} className="w-8 h-8 bg-naukri-green rounded-full flex items-center justify-center border-2 border-white hover:scale-110 transition-transform duration-300" style={{ animationDelay: `${i * 0.1}s` }}>
