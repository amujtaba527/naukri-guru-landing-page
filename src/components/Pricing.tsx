      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <div 
              key={index} 
              className={`relative bg-white rounded-2xl border-2 p-8 transition-all duration-300 hover:shadow-lg animate-fade-in ${
                plan.popular 
                  ? 'border-naukri-green shadow-lg scale-105' 
                  : 'border-gray-100'
              }`}
            ...
            >
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                  <span className="bg-naukri-amber text-black px-4 py-2 rounded-full text-sm font-semibold">
                    Most Popular
                  </span>
                </div>
              )}
              ...
              <div className="space-y-4 mb-8">
                {plan.features.map((feature, featureIndex) => (
                  <div key={featureIndex} className="flex items-center gap-3">
                    <Check className="h-5 w-5 text-naukri-green flex-shrink-0" />
